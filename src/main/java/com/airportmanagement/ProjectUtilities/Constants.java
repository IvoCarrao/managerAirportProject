package com.airportmanagement.ProjectUtilities;

public abstract  class Constants {

    public static String REQUEST_IS_NULL_ERROR = "Request is null";
    public static String REQUEST_EMPTY_ERROR = "Request without a request type or a body";
    public static String YEAR_INVALID_ERROR = "Airplane with invalid release year";
    public static String BRAND_INVALID_ERROR = "Brand has to exist";
    public static String INVALID_ID_ERROR = "Invalid ID";
    public static String VALID_REQUEST = "Request is valid";
    public static String INVALID_QUERY_PARAMETER = "Invalid query parameter";
    public static String INVALID_CITY_NAME = "Invalid city name";
    public static String INVALID_AIRPORT_NAME = "Invalid airport name";

}
